...
export function ListaFiltrable() {
    ...
    let total = 0;
    for(let i = 0; i < 5000000; i ++); //Simulación de carga intensa
    ...
    // useEffect: Auditamos cambios en los resultados fultrados
    useEffect(() =>{
    ...
    return (
        ...
        <div>
            ...
            <button onClick={() => setContadorInutil(c => c + 1)}>Re-renderizar Componente({contadorInutil})</button>
        </div>
        <p>Resultados encontrados: <strong>{filtrados.length}</strong> de {items.lenght}</p>
        <ul>
